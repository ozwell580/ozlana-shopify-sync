...
def list_local_subfolders(code):
    code_dir = os.path.join(IMAGES_DIR, code)
    if not os.path.isdir(code_dir):
        return {}
    result = {}
    for name in sorted(os.listdir(code_dir)):
        sub_dir = os.path.join(code_dir, name)
        if not os.path.isdir(sub_dir):
            continue
        files = sorted(f for f in os.listdir(sub_dir) if f.lower().endswith(IMAGE_EXTENSIONS))
        if files:
            result[name] = files
    return result
